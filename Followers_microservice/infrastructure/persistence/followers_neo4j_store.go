...
func (store *FollowersStore) GetFollows(id string) ([]*domain.User, error) {
	...
	followers, err := session.ReadTransaction(func(tx neo4j.Transaction) (interface{}, error) {
		records, err := tx.Run(
			"MATCH (:User {userId:$id})-[:FOLLOWING]->(followed:User) RETURN followed",
...
func (store *FollowersStore) GetFollowers(id string) ([]*domain.User, error) {
	...
	followers, err := session.ReadTransaction(func(tx neo4j.Transaction) (interface{}, error) {
		records, err := tx.Run(
			"MATCH (:User {userId:$id})<-[:FOLLOWING]-(follower:User) RETURN follower",
			map[string]interface{}{"id": id})
		if err != nil {
			return nil, err
		}
...
func (store *FollowersStore) GetFollowRequests(id string) ([]*domain.User, error) {
	...
	followers, err := session.ReadTransaction(func(tx neo4j.Transaction) (interface{}, error) {
		records, err := tx.Run(
			"MATCH (:User {userId:$id})-[:REQUESTING_FOLLOW]->(followed:User) RETURN followed",
			map[string]interface{}{"id": id})
		if err != nil {
			return nil, err
		}
...
func (store *FollowersStore) GetFollowerRequests(id string) ([]*domain.User, error) {
	...
	followers, err := session.ReadTransaction(func(tx neo4j.Transaction) (interface{}, error) {
		records, err := tx.Run(
			"MATCH (:User {userId:$id})<-[:REQUESTING_FOLLOW]-(follower:User) RETURN follower",
			map[string]interface{}{"id": id})
		if err != nil {
			return nil, err
		}
