...
import SelectBoxCompany from "./SelectBoxCompany"
...
const CreateJobOffer =  () =>{
    ...
    const companySelector = companyId => setCompanyId(companyId);

    const Validate = () => {
        if (position === "" || description === ""|| requirements === "" || companyId === "") {
            Swal.fire({
                icon: 'warning',
                title: 'Oops...',
    ...
    const addJobOffer = async (e)=>{
        ...
        axios.post(axios.defaults.baseURL + 'api/Companies/JobOffer', newOffer, {headers})
                .then(res => {
                    console.log(res.data)
                    history('/jobOffers/' + companyId)
                }).catch(err => {
                    console.log(err);
                    Swal.fire({
                        icon: 'error',
                    ...
                    <form className="form-horizontal">
                            ...
                            <div className="form-group">
                                <div className="col-md-8">
                                    <SelectBoxCompany className="form-control" selectedValue={companyId} setSelectedValue={companySelector} path={"api/Companies/User/" + localStorage.getItem('user_id')} message={"Choose company"}/>
                                </div>
                            </div>
